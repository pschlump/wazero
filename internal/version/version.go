...
//
// If this is not CLI, this assumes that downstream users of wazero imports wazero as "github.com/pschlump/wazero".
// To be precise, the returned string matches the require statement there.
// For example, if the go.mod has "require github.com/pschlump/wazero 0.1.2-12314124-abcd",
...
func init() {
	...
	if ok {
		for _, dep := range info.Deps {
			// Note: here's the assumption that wazero is imported as github.com/pschlump/wazero.
			if strings.Contains(dep.Path, "github.com/pschlump/wazero") {
				version = dep.Version
			}
		}
